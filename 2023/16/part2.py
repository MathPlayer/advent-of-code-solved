...
def solve(filename):
    ...
    best = -1
    for row in range(rows):
        energized = energize(grid, rows, cols, (row, 0), R)
        best = max(best, len(energized))

        energized = energize(grid, rows, cols, (row, cols - 1), L)
        best = max(best, len(energized))

    for col in range(cols):
        energized = energize(grid, rows, cols, (0, col), D)
        best = max(best, len(energized))
        energized = energize(grid, rows, cols, (rows - 1, col), U)
        best = max(best, len(energized))
